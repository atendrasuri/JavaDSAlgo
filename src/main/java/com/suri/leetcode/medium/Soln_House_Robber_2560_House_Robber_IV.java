 ...
 */
public class Soln_House_Robber_2560_House_Robber_IV {
    ...
    public static int minCapability(int[] nums, int k) {
        ...
        while(left<right){
            ...
            if(canRobWithMaxValue(nums, k, mid)){
                right = mid;
            }else{
                left = mid+1;
            }
    ...
    public static boolean canRobWithMaxValue(int []nums, int k, int maxValue){
        int i=0;
        int count =0;
        while(i<nums.length){
            if(nums[i]<=maxValue){
